#!/usr/bin/env python3
"""Generate a benchmark notebook for question-level profiling."""
...
OUTPUT_PATH = REPO_ROOT / "notebooks" / "Junior_Question_Benchmark.ipynb"
...
def build_notebook():
    ...
    nb["cells"] = [
        md(
            ...
            "This notebook runs the `turboquant-rag` pipeline over a user-defined "
            "set of questions from `questions.json` and records the profiling "
            "metrics used in the Junior profiling experiment:\n\n"
        ...
        md(
            ...
            "Set `BACKEND` to `turboquant` for the compressed KV-cache run, or `bf16` "
            "for the baseline if your GPU has enough memory."
        ),
        code(
            "from pathlib import Path\n"
            "import json\n"
        ...
        md(
            ...
            "This notebook assumes your `.env` file already points to the Llama "
            "weights and tokenizer.\n\n"
            "If you are running BF16, you may need to reduce `max_seq_length` or "
            "use a larger GPU."
        ),
        code(
            ...
            "    if backend == 'bf16':\n"
            "        return LlamaBF16(max_seq_length=MAX_SEQ_LENGTH, batch_size=1, device='cuda', is_batch=True)\n"
        ...
        md(
            ...
            "If `data/corpus.json` is not available, the notebook falls back to a tiny "
            "demo corpus so the flow still runs end-to-end."
        ),
        code(
        ...
        md(
            ...
            "If `reference_answer` is present, the result rows will also include exact match "
            "and token-level F1."
        ),
        code(
        ...
        md(
            "## 5. Run the Benchmark\n\n"
            "Each question run records the metrics from the profiling notebook and appends "
            "a JSONL row under `results/question_benchmark/`."
        ),
        code(
        ...
        md(
            "## 6. Summarize Results\n\n"
            "The notebook prints a compact table of the key metrics and the aggregate "
            "means across the selected questions."
        ),
        code(
            "import pandas as pd\n"
