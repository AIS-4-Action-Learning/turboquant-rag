...
class Attention(nn.Module):
    def __init__(
        ...
        if self.use_compressed_kv_cache:
            ...
            self.cache_k = torch.zeros((args.max_batch_size, args.max_seq_len, self.n_local_kv_heads, self.head_dim), device="meta")
            self.cache_v = torch.zeros((args.max_batch_size, args.max_seq_len, self.n_local_kv_heads, self.head_dim), device="meta")
    ...
    def _allocate_cache(self, compressor, args):
        ...
        return (
            torch.zeros((*shape, b_bytes), dtype=torch.uint8, device="meta"),
            torch.zeros((*shape, q_bytes), dtype=torch.uint8, device="meta"),
            torch.zeros(shape, dtype=torch.float32, device="meta"),
            torch.zeros(shape, dtype=torch.float32, device="meta")
        )
